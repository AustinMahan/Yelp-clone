  ...
  return knex('restaurants').del()
    .then(function () {
      return Promise.all([
        ...
        knex('restaurants').insert({name: faker.company.companyName(), type: types[Math.floor(Math.random() * 4)], description: faker.lorem.paragraph(), location: `${faker.address.streetAddress()}, ${faker.address.city()}, ${faker.address.stateAbbr()}`, url: faker.image.food(), avg_review: (knex('reviews').avg('rating').where('restaurant_id', 13))})
      ]);
    });
};
